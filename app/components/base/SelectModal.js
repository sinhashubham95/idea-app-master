...
const NativeModal = class extends React.Component {
    ...
    render() {
        const { isLoading, multiple, items, placeholder, filterItem, renderRow, isSelected, required, keyExtractor } = this.props;
        const { onChange } = this;
            ...
            <Flex style={[Styles.body]}>
                ...
                {items && (
                    <Fade style={{ flex: 1 }} autostart value={1}>
                        <Select
                          placeholder={placeholder || localizedStrings.search}
                          items={items}
                          value={value}
                          onChange={onChange}
                          multiple={multiple}
                ...
                <FormGroup>
                    <Column>
                        <ButtonPrimary style={Styles.mb5} onPress={this.onDone} disabled={required ? !value : false}>{localizedStrings.done}</ButtonPrimary>
                    </Column>
                </FormGroup>
            </Flex>
        );
